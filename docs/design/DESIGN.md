...
**Version**: v2.1 (post-API-pivot · 개인매물 분기 반영)
**Date**: 2026-04-09
**Status**: MVP 동작 확인 — fem.encar.com 매물에서 11 룰 전부 평가 (딜러 + 개인매물 양쪽)
...
│  │                   index.ts (orchestrator — httpStatus-aware resolveApi)
│  ├─ parsers/utils/   text.ts (wonToNumber, splitLines)
│  ├─ collectors/      sources.ts (SourceId registry + priority)
│  │                   merge.ts (mergeFieldStatus primitive — conflict detect)
│  │                   README.md (layer architecture notes)
│  ├─ bridge/          encar-to-facts.ts (F1–F5 불변식 + isPersonalListing)
│  ├─ rules/           r01..r11 + evaluate + REASON_LABEL 한글 매핑
│  ├─ storage/         db.ts (Dexie 4 tables + sweepExpired)
│  └─ messaging/       protocol.ts (Message union + FetchStatus)
├─ background/
...
| `api.encar.com/v1/readside/inspection/vehicle/{vid}` | 성능점검 구조화 (master.{accdient, simpleRepair, detail.{waterlog, recall, tuning}}, outers[].{type, statusTypes}) | R02 보조, R06 교차검증 | X |

> **Sample 005 발견**: `inspection` 응답의 `outers[].statusTypes` (교환/판금/부식 등) 가 `record.accidents[].detailList` 의 부위별 동일 사건을 두 관점에서 보여주므로 향후 교차검증 소스로 사용 가능. `master.detail.waterlog` 는 R06 침수 단일 플래그 — 기존 `recordApi.floodTotalLossCnt` 와 merge 대상.

#### 3.1a 개인(CLIENT) 매물 엔드포인트 가용성

Sample 006 실측으로 확인된 딜러·개인 간 엔드포인트 차이:

| 엔드포인트 | 딜러 매물 | 개인(CLIENT) 매물 |
|---|---|---|
| `__PRELOADED_STATE__.cars.base` | ✅ | ✅ (단 `vin=null`, `tradeType=null`) |
| `__PRELOADED_STATE__.cars.detailFlags` | ✅ (`isDealer=true`) | ✅ (`isDealer=false`) |
| `/readside/record/vehicle/{id}/open` | ✅ | **✅** (41707401 실측, 4사고 + 보험공백 2건 반환) |
| `/readside/diagnosis/vehicle/{id}` | ✅ (진단받은 차량) | ❌ **HTTP 404** |
| `/readside/inspection/vehicle/{id}` | ✅ (진단받은 차량) | ❌ **HTTP 404** |

→ 개인매물은 엔카진단/성능점검 API 가 원천 부재. R03/R04 는 KILLER 로 떨어뜨리면 안 되고, `FieldStatus.parse_failed('not_applicable_personal')` 로 라우팅해 severity `unknown` 처리. 이 규칙은 **F5 불변식** 으로 문서화 (§5.3).

관측된 `base.contact.userType` 값 카탈로그: `'CLIENT' | 'DEALER'` 둘. `'PERSONAL'`, `'PRIVATE'` 등은 관측되지 않음.
관측된 `base.category.importType` 값: `'REGULAR_IMPORT' | 'BRAND_IMPORT' | 'NONE_IMPORT_TYPE'` (`NONE_IMPORT_TYPE` 은 개인매물 sample 006 에서 신규).
...
[2] main-world content script 주입 (run_at: document_idle, world: 'MAIN')
     ...
     │  • 각 fetch 의 HTTP 상태를 FetchStatus 로 별도 집계
     │    (ok | not_found | unauthorized | error | skipped)
     │    → 404(개인매물) ≠ 네트워크 에러 구분
     │
     ▼  window.postMessage
...
[4] Background service worker
     ...
     │  • orchestrate(resolveApi) → HTTP 404 → 'no_report_for_personal'
     │                              HTTP 401/403 → 'login_required'
     │                              network/5xx → 'api_fetch_error'
     │  • encarToFacts(isPersonalListing branch) → evaluate
     │  • cache.put
     │
...
| R03 | 엔카진단 통과 | detailFlags.isDiagnosisExist | **KILLER** (딜러만) | dealer+true=pass / dealer+false=killer / **personal=unknown** (F5) |
| R04 | 프레임 무사고 | diagnosisApi CHECKER_COMMENT → fallback hasEncarDiagnosis | **KILLER** (딜러만) | false=pass / true=killer / personal=unknown (diagnosis 404) |
...
- **F5**: **딜러 vs 개인 매물은 분기(branch) 다, 속성이 아니다.** `isPersonalListing(base, flags)` 가 true 일 때:
  - R03 `hasEncarDiagnosis` 는 `parse_failed('not_applicable_personal')` → severity `unknown` (KILLER 아님)
  - R04 `frameDamage` 도 diagnosisApi / inspectionApi 가 404 로 부재하므로 `'not_derived'` → `unknown`
  - R05~R10 은 `recordApi` 가 개인매물에도 정상 반환되므로 **그대로 동작**
  - bridge 는 `bridgeWarnings` 에 `'personal_listing'`, `'r03_skipped_personal'` 기록
  - *Sample 006 (BMW G30 개인매물) 이 회귀 앵커 — 이 매물은 R08 진짜 killer 로 NEVER 지만 R03 은 unknown 이어야 함.*
  - 판별 규칙 (우선순위 순): `detailFlags.isDealer === false` → personal / `detailFlags.isDealer === true` → dealer / 둘 다 없으면 `contact.userType === 'CLIENT'` / 그 외엔 dealer 가정 (안전 기본값).

### 5.4 Coverage Matrix v2

| 룰 | 필요 소스 | 로그인 | 딜러 신뢰도 | 개인 신뢰도 |
|---|---|---|---|---|
| R01 | detailFlags | ❌ | 🟢 | 🟢 |
| R02 | detailFlags | ❌ | 🟢 | 🟢 (isHistoryView=true 여도 inspection API 는 404) |
| R03 | detailFlags | ❌ | 🟢 pass/killer | ⚪ **F5 → unknown (N/A)** |
| R04 | diagnosisApi (+ R03 fallback) | ❌ | 🟢 | ⚪ unknown (diagnosis 404) |
| R05 | recordApi | ❌ | 🟢 | 🟢 (record API 는 개인매물도 반환) |
| R06 | recordApi | ❌ | 🟢 | 🟢 |
| R07 | recordApi | ❌ | 🟢 | 🟢 |
| R08 | recordApi | ❌ | 🟢 | 🟢 |
...
**딜러 매물**: 모든 11 룰이 로그인 없이 동작. **개인매물**: R03/R04 는 원천 데이터 부재로 unknown, 나머지 9개 룰은 정상 동작. → 개인매물의 verdict 범위는 `NEVER | CAUTION | UNKNOWN` (never `OK` — R03 이 unknown 이라 OK 로 갈 수 없음).
...
| 컬렉터 merge | tests/collectors.test.ts | 15 | SOURCE_REGISTRY priority + mergeFieldStatus 정책 |
| 통합 (fixture) | tests/integration.test.ts | 7 | parsed → facts → verdict 종단 (딜러 4 + 개인 2 + ideal 1) |

**총 38 테스트**, Vitest + happy-dom 불필요 (파서가 DOM 의존 없음).

### 9.2 Fixture 기반 회귀

`src/__fixtures__/samples.ts` 에 5 실 샘플 + 2 synthetic 고정:

| 샘플 | 차량 | 판매자 | 예상 verdict | 핵심 검증 |
|---|---|---|---|---|
| 001 | 기아 스포티지 5세대 | 딜러 | NEVER (R05, R08) | loan>0 + notJoinDate1 |
| 002 | 현대 팰리세이드 | 딜러 | NEVER (R03, R05, R08) | isDiagnosisExist=false |
| 003 | 현대 팰리세이드 2.2 | 딜러 | NEVER (R03, R05, R08) + R10 WARN | otherAccidentCost > 100만 |
| 004 | BMW E90 328i | 딜러 | NEVER (R08) + R10 WARN | **F1 불변식 — 법인 ≠ 렌트** |
| 006 | BMW G30 530i | **개인(CLIENT)** | NEVER (R08) + R10 WARN | **F5 불변식 — R03 은 unknown, NEVER 는 오직 R08 로만** |
| PersonalClean | 합성 개인 아반떼 | 개인(CLIENT) | UNKNOWN | 완벽 클린이어도 OK 아님 (R03 unknown) |
| ideal | 합성 딜러 아반떼 2024 | 딜러 | OK | 전체 클린 |

Verdict 불일치 발생 시 즉시 회귀로 잡힘. 특히 006 픽스처는 **개인매물이 R03 KILLER 로 잘못 떨어지는 회귀**의 카나리아다.
...
| 13 | 딜러 vs 개인매물 분기 | 속성 플래그 / 분기 로직 / 별도 파이프라인 | **bridge 내부 분기 (F5)** | Sample 006 증명 — 개인은 diagnosis/inspection 원천 부재이므로 R03 을 KILLER 로 유지하면 모든 개인매물이 잘못 NEVER 로 판정됨. 분기는 한 곳(`isPersonalListing`) 에서만 수행, 나머지 룰은 재사용. |
| 14 | HTTP 404 vs 네트워크 에러 | 통합 parse_failed / 구분 | **구분 (FetchStatus)** | collector 단에서 404 를 `'no_report_for_personal'`, 네트워크를 `'api_fetch_error'` 로 라우팅. UI 에서 "개인매물 — 엔카 리포트 없음" vs "데이터 수집 오류" 로 다르게 표시. |
| 15 | collectors 레이어 | parsers 에 병합 / 전용 레이어 | **전용 `src/core/collectors/`** | 교차검증 merge 로직(예: `inspection.waterlog` vs `record.floodTotalLossCnt`) 이 parser 에 섞이면 단일 책임 위반. merge.ts 는 순수 함수로 분리. |
...
│  ├─ discovery/encar/
│  │  ├─ README.md           통합 디스커버리
│  │  └─ samples/001~006.md  6 실 매물 상세 (004=법인 ≠ 렌트, 005=inspect API 스키마, 006=첫 개인매물)
│  └─ plan/
│     ├─ IMPLEMENTATION.md
│     ├─ REVIEW.md           계획 리뷰
...
npx vitest run       # 38/38 통과 (parsers 14 + orchestrate 2 + collectors 15 + integration 7)
